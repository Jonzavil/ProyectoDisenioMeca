...
def generate_launch_description():
    ...
    s_description_dir = get_package_share_directory("s_description")
    s_description_share = os.path.join(get_package_prefix("s_description"), "share")
    gazebo_ros_dir = get_package_share_directory("gazebo_ros")

    # Argumento de lanzamiento para especificar la ubicación del archivo URDF del robot
    model_arg = DeclareLaunchArgument(
        name="srbot",
        default_value=os.path.join(s_description_dir, "urdf", "sbot.urdf.xacro"),
        description="Absolute path to robot urdf file"
    )

    # Declaración del archivo del mundo de Gazebo
    world_file_name = 'ramel.world'
    world_path = os.path.join(s_description_dir, 'worlds', world_file_name)
    ...
    env_var = SetEnvironmentVariable("GAZEBO_MODEL_PATH", s_description_share)

    # Configuración del parámetro del nodo del robot_state_publisher
    robot_description = ParameterValue(Command(["xacro ", LaunchConfiguration("srbot")]), value_type=str)
    ...
    spawn_robot = Node(
        ...
        arguments=["-entity", "srbot", "-topic", "robot_description"],
        output="screen"
    )
